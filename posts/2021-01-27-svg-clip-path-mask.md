...
Scalable Vector Graphics, or SVG, are one of those front-end tools that can open up a world of creative ideas. I've been playing around with it more, for reasons of work and distractions from my hollow, quarantined soul, and made my first component with some cutesy SVG animation.
...
But near the end, I hit a snag with how SVG can hide parts of the image. Sometimes I wanted to *show* specific parts of an image. Other times I wanted to *hide* specific parts of one. I couldn't get this effect for the sun and moon animations, so I got it with background colors instead. Not ideal, but I can always try again later.

I wanted to get it right next time, but I had a hard time finding a clear explanation of showing and hiding SVG. So I decided to write it myself! **The two parts of SVG one needs for these effects are `clip-path` and `mask`.** Here I'll break down the basics of how to use them with SVG to hide shapes, images, and painful tangles of emotion.

## Clip-path Controls What You See

Let's say I have a red rectangle, but I only want to show a small circle *within* it. That's when I need `clip-path.` **A `clip-path` is like another SVG shape you pair with a second shape. Anything in the second shape that doesn't overlap with the `clip-path` gets cropped out, lost, and never seen again.** You destroyed it and I hope you're happy.
...
Let's add a `clip-path` to this thing. First, I need to define the circle it'll be using. It's positioned to be in the rectangle's center.

```html
<circle cx="0" cy="0" r="50" />
```

Next, I need to define it as a `clip-path`. **This has to go within a `defs` element, which is SVG's way of storing variables.** Anything in `defs` won't directly render but can then affect other elements, like repressed memories of staring wistfully at an empty street after the third hard cider.

Within the `defs` element, the `circle` must go in a `clipPath` element. It also needs a unique ID for reference.
...
Now, this circle is a `clip-path` and can go with the red rectangle. It needs the `clip-path` attribute with the unique ID, which looks like `clip-path="url(#clip-path)"`. Now it's good to go!
...
Once I finally peeled myself off the couch and keep working, I asked: what if I want the reverse result: to hide everything *except* that red dot? To do that, we need a `mask`. But this approach is a little more complex.
...
This is like a `clip-path` but with a lot more potential for what to show and hide, but will only realize once it can finally experience regular human interaction. But I only needed these lonely basics to get the result I want. If you want to see what it can do, including using shades of gray for translucent effects, check out [this blog post of mask examples](https://vanseodesign.com/web-design/svg-masking-examples-1/).

First, defining a visible area here is easy: I want the rectangle to be visible! I'm hiding space within the rectangle, so the rectangle itself should be my visible area. I define this within a `mask` element, which can also go in the `defs` element.
...
Notice the `fill="white"` I added to the shape. That's important because, **in a mask, the visible area must have a white color fill.** Don't you dare deprive it of what it needs. You monster.

Now I need to decide what area to hide within the visible area. All I need is the circle that goes in the middle of the rectangle. This time it needs `fill="black"` to hide it.
...
This gives me what I wanted, a rectangle missing the center circle! The result is below with the light yellow again showing what's hidden behind the mask. The SVG mask anyway, not the mask covering up the crushing conflict between talking face to face and a small yet serious health risk that's already taken away so many.
...
Rereading this post, I think the second part may be me using learning to handle the mental stress of extended quarantine. But the point about making coolor code still works.

So focus on that. I need to talk a long walk. To the wine shop.
